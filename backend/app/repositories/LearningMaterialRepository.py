...
from ..schemas import LearningMaterialCreate, LearningMaterialUpdate  # Assuming these schemas are defined

class LearningMaterialRepository:
    def __init__(self, db: Session):
        self.db = db

    def create_learning_material(self, learning_material: LearningMaterialCreate) -> LearningMaterial:
    ...
    def get_learning_material(self, learning_material_id: UUID) -> Optional[LearningMaterial]:
        return self.db.query(LearningMaterial).filter(LearningMaterial.id == learning_material_id).first()

    def get_learning_materials(self, skip: int = 0, limit: int = 100) -> List[LearningMaterial]:
    ...
    def update_learning_material(self, learning_material_id: UUID, learning_material_update: LearningMaterialUpdate) -> Optional[LearningMaterial]:
        db_learning_material = self.db.query(LearningMaterial).filter(LearningMaterial.id == learning_material_id).first()
