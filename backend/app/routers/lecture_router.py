...
from dependencies import get_lecture_service
...
def get_lecture(lecture_id: UUID, service: LectureService = Depends(get_lecture_service)):
    try:
...
@router.get("/lectures/by_summarization/{summarization_id}", response_model=Lecture)
def get_lecture_by_summarization_id(summarization_id: UUID, service: LectureService = Depends(get_lecture_service)):
    try:
...
def create_lecture(lecture_data: CreateLectureDTO, service: LectureService = Depends(get_lecture_service)):
    return service.create_lecture(lecture_data)

@router.put("/lectures/{lecture_id}", response_model=Lecture)
def update_lecture(lecture_id: UUID, lecture_data: UpdateLectureDTO, service: LectureService = Depends(get_lecture_service)):
    try:
...
def delete_lecture(lecture_id: UUID, service: LectureService = Depends(get_lecture_service)):
    try:
